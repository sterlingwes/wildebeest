	...
	const { results: following } = await db
		.prepare(
            ...
            SELECT
                ...
                json_extract_path(actors.properties::json, 'followers') as actorFollowersURL
...
WHERE
     objects.type = 'Note'
     AND outbox_objects.actor_id IN (SELECT value::text FROM json_array_elements(?2))
     AND json_extract_path(objects.properties::json, 'inReplyTo') IS NULL
     AND (outbox_objects.target = '${PUBLIC_GROUP}' OR outbox_objects.target IN (SELECT value::text FROM json_array_elements(?3)))
GROUP BY objects.id, actors.id, outbox_objects.actor_id, outbox_objects.published_date
...
WHERE objects.type='Note'
      ...
      ${hashtagFilter}
GROUP BY objects.id, actors.id, actors.cdate, actors.properties, outbox_objects.actor_id, outbox_objects.published_date
ORDER by outbox_objects.published_date DESC
LIMIT ?1 OFFSET ?2
`
	const DEFAULT_LIMIT = 20
